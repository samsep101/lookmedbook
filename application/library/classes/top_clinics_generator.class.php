<?php
	class TopClinicsGenerator extends YandexServicesCompaniesFeedGenerator
	{
        ...
        public function static_clinic_validate($clinic)
        {
            ...
            else if ($clinic->just_its_specialties && !$clinic->not_virtual_doctors)
            {
                ...
                {
                    if (!$doctor_specialty_to_clinic->doctor->is_virtual) {
		...
		private function createClinicNode(ClinicModel $clinic)
		{
            ...
            /* По требованию заказчика устанавливаем указанный им телефон */
//            if ($clinic->phones)
//            {
...
//            }
            $phone_node = $dom->createElement('phone');
            $phone_node->appendChild($dom->createElement('type', 'phone'));
            $phone_node->appendChild($dom->createElement('number', $phoneNumber));
            ...
            if ($clinic->reviews)
            {
                foreach($clinic->reviews as $review)
                {
                    ...
                    $vcard_node->appendChild($dom->createElement('fn', $review->getAccountNameOrFallback('nick')));
            ...
            if(!$this->static_service_only_flag) {
                ...
                if ($clinic->not_virtual_doctors)
                {
                    ...
                    foreach($clinic->not_virtual_doctors as $doctor)
                    {
                        ...
                        $firstVisitPrice = $doctor->getFirstVisitPriceByClinicId($clinic->getId());
                        $secondVisitPrice = $doctor->getSecondVisitPrice($clinic->getId());

                        if($firstVisitPrice) $resource_node->appendChild($this->dom_document_instance->createElement('first-visit-price', $firstVisitPrice));
                        if($secondVisitPrice) $resource_node->appendChild($this->dom_document_instance->createElement('second-visit-price', $secondVisitPrice));

                        if($doctor->about) $resource_node->appendChild($dom->createElement('about', $this->replaceIllegalCharacters($doctor->about)));
                        if($doctor->specialties_names) $resource_node->appendChild($dom->createElement('specialties-names', $doctor->specialties_names));
                        if($doctor->education) $resource_node->appendChild($dom->createElement('education', $this->replaceIllegalCharacters($doctor->education)));
                        if($doctor->course) $resource_node->appendChild($dom->createElement('course', $this->replaceIllegalCharacters($doctor->course)));
                        if($doctor->certificate) $resource_node->appendChild($dom->createElement('certificate', $this->replaceIllegalCharacters($doctor->certificate)));
                        if($doctor->academic_title) $resource_node->appendChild($dom->createElement('academic-title', $this->replaceIllegalCharacters($doctor->academic_title)));
                        if($doctor->rate) $resource_node->appendChild($dom->createElement('rate', $doctor->rate));
                        if($doctor->work_experience) $resource_node->appendChild($dom->createElement('work-experience', $doctor->work_experience));
